...
def contact(request):
    if request.method=="POST":
        ...
        myquery = Contact(name = name, email = email , phone = number, description = desc)
        myquery.save()       
...
def enroll(request):
    ...
    if request.method=="POST":
        ...
        query = Enrollment(FullName=FullName,Email=email,Gender=gender,PhoneNumber=PhoneNumber,SelectMembershipplan=member,SelectTrainer=trainer,Address=address)
        query.save()
